...
$Path = "C:\2\ClientInventory\$newfolder\Desktop.csv"
...
Get-ADComputer  -Properties * -Filter {LastLogonDate -ge $cutoffdate} -SearchBase "dc=$domain,dc=sunbeam,dc=com" -Server $domaincontroller | Select Name,OperatingSystem,OperatingSystemVersion,LastLogonDate,CanonicalName,distinguishedname|Export-Csv -Path $Path -Append
#-----------------------------------------------------------------------------------------
#SunbeamProducts
#-----------------------------------------------------------------------------------------
